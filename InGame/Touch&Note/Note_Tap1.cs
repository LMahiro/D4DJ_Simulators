...
public class Note_Tap1 : Note
{
    private void Start()
    {
        ...
        NoteState = new Tap1_WaitingForJudgment();
...
// 等待判定
public class Tap1_WaitingForJudgment : INoteState
{
    ...
    public void UpdateState(Note note)
    {
        if (note.IsAUTO)
        {
            if (note.Event.time - GameStateManager.Instance.GameTime <= 0)
            {
                ...
                InGameSoundManager.Instance.PlaySe_Note(InGameSe_Note.Tap1_Perfect);
...
// 判定中
public class Tap1_JudgmentInProgress : INoteState
{
    public void EnterState(Note note)
    {
    ...
    public void ExitState(Note note)
    {
        ...
        if (note.DelayTime < -note.GoodTime)
        {
            ...
            InGameSoundManager.Instance.PlaySe_Note(InGameSe_Note.Tap1_Bad);
        }
        else if (note.DelayTime < -note.GreatTime)
        {
            note.DisplayEffects(0);
            note.UICommunication(JudgmentText.Fast_Good, 1);
            InGameSoundManager.Instance.PlaySe_Note(InGameSe_Note.Tap1_Good);
        }
        else if (note.DelayTime < -note.PerfectTime)
        {
            note.DisplayEffects(0);
            note.UICommunication(JudgmentText.Fast_Great, 1);
            InGameSoundManager.Instance.PlaySe_Note(InGameSe_Note.Tap1_Great);
        }
...
// 判定结束
public class Tap1_JudgmentEnded : INoteState
{
    public void EnterState(Note note)
    {
