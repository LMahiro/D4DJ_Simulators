...
// 适合多个Toggle切换Enum
public class Settings_GameInterfaceFramerate : MonoBehaviour
{
    ...
    {
        // 从数据持久化类中获取保存的Enum值
        Framerate savedEnumValue = GameSettingsMannger.save_Settings.gameInterfaceFramerate;
    ...
    private void OnToggleValueChanged(Toggle toggle)
    {
        ...
        if (toggle == toggleOption1)
            GameSettingsMannger.save_Settings.gameInterfaceFramerate = Framerate._30;
        else if (toggle == toggleOption2)
            GameSettingsMannger.save_Settings.gameInterfaceFramerate = Framerate._60;
        else if (toggle == toggleOption3)
            GameSettingsMannger.save_Settings.gameInterfaceFramerate = Framerate._120;
