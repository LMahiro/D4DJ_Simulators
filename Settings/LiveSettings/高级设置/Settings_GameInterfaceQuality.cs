...
// 适合多个Toggle切换Enum
public class Settings_GameInterfaceQuality : MonoBehaviour
{
    ...
    {
        // 从数据持久化类中获取保存的Enum值
        QualityLevel savedEnumValue = GameSettingsMannger.save_Settings.gameInterfaceQuality;
    ...
    private void OnToggleValueChanged(Toggle toggle)
    {
        ...
        if (toggle == toggleOption1)
            GameSettingsMannger.save_Settings.gameInterfaceQuality = QualityLevel.High;
        else if (toggle == toggleOption2)
            GameSettingsMannger.save_Settings.gameInterfaceQuality = QualityLevel.Medium;
        else if (toggle == toggleOption3)
            GameSettingsMannger.save_Settings.gameInterfaceQuality = QualityLevel.Low;
